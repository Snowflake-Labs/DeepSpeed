...
def matmul_fp8(inp, weight, scale, quantization_group_size):
    ...
    BLOCK_SIZE_N = 64
    BLOCK_SIZE_K = min(max(64, quantization_group_size), 128)
    GROUP_SIZE_M = 8
    num_stages = 4
    num_warps = 4
    if M >= 256:
    ...
    kernel = (matmul_kernel_fp8_bf16 if quantization_group_size == weight.numel() else groupwise_matmul_kernel_fp8_bf16) if inp.dtype == torch.bfloat16 else matmul_kernel_fp8_fp16
    kernel[grid](inp,
                 weight,
                 out,
                 scale,
