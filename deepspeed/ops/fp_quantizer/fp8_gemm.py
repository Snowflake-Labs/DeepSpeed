...
@triton.jit
def matmul_kernel_fp8_bf16(inp_ptr, weight_ptr, out_ptr, scale, M, N, K, stride_am, stride_ak, stride_bk, stride_bn,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        w = w.to(tl.bfloat16, bitcast=True)
    ...
    out = (accumulator * scale).to(tl.bfloat16)
...
@triton.jit
def groupwise_matmul_kernel_fp8_bf16(inp_ptr, weight_ptr, out_ptr, scale_ptr, M, N, K, stride_am, stride_ak, stride_bk,
                                     stride_bn, stride_cm, stride_cn, BLOCK_SIZE_M: tl.constexpr,
                                     BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr,
                                     GROUP_SIZE_M: tl.constexpr, quantization_group_size: tl.constexpr):
    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(M, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    num_pid_in_group = GROUP_SIZE_M * num_pid_n
...
def matmul_fp8(inp, weight, scale, quantization_group_size):
    ...
    kernel = (matmul_kernel_fp8_bf16 if quantization_group_size == -1 else
              groupwise_matmul_kernel_fp8_bf16) if inp.dtype == torch.bfloat16 else matmul_kernel_fp8_fp16
    kernel[grid](inp,
                 weight,
                 out,
                 scale,
