...
from .container import DeepSeekNonTransformerContainer, DeepSeekTransformerContainer


class DeepSeekInferenceModel(DSMoETransformerModelBase):
    """
    Inference model implementation for Mixtral models.
    """

    _non_transformer: Optional[DeepSeekNonTransformerContainer]
    """
    Embed + unembed container. Specializing the type annotation.
    """

    _transformer: Optional[Iterable[DeepSeekTransformerContainer]]
    """
    ...
    @property
    def n_experts_groups(self) -> int:
        return self._config.n_group

    @property
    def n_group_top_k(self) -> int:
        return self._config.num_experts_per_group
    ...
    def _forward_transformer(self, layer_idx: int, residual: torch.Tensor, hidden_states: torch.Tensor,
        ...
        latent_cache = self.state_manager.get_cache(layer_idx)
        latent_state = self.kv_a_proj_with_mqa(hidden_states)
        if self.q_lora_rank is not None:
            q = self.q_lora_proj1(hidden_states, cur_params.q_lora_w1)
            q = self.q_lora_norm(q, cur_params.q_lora_norm_gamma)
            # a = torch.randn(m,h)
            # b = torch.randn(h,o)
            # c = torch.randn(m,ld)
            # d = torch.randn(ld,o)
            # k = torch.matmul(d, b.T)
            # torch.matmul(a,torch.matmul(c, k).T) = torch.matmul(torch.matmul(a,b),torch.matmul(c,d).T)
            q = self.qk_merged_lora_proj2(latent_state, cur_params.qk_merged_lora_w2)
        else:
            q = self.qk_proj(latent_state, cur_params.qk_merged_w)
            
        v_o = self.v_attn_o(latent_state, cur_params.v_attn_out_w)
        hidden_states = self.latent_attn(q, hidden_states, v_o, latent_cache, ragged_batch_info)
